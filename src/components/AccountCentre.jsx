...
// ── AccountCentre ─────────────────────────────────────────────────────────────
export default function AccountCentre({ user, profile, tier, onClose, onNavigate }) {
    ...
    <div className="acc-overlay" onClick={onClose}>
      <div className="acc-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="acc-body">
          ...
          {section === 'uploads'      && <UploadsSection user={user} />}
...
function SubscriptionSection({ tier, onClose, onNavigate }) {
  ...
  return (
    <div className="acc-section">
      ...
      {sub.billingCycleEnd && (
        <p className="acc-hint">Billing cycle ends: {fmtDate(sub.billingCycleEnd)}</p>
      )}
      {sub.nextBillingDate && !sub.cancelAtPeriodEnd && (
        <p className="acc-hint">Next renewal: {fmtDate(sub.nextBillingDate)}</p>
      )}
      ...
      {sub.scheduledTier && !sub.cancelAtPeriodEnd && sub.scheduledTier !== tier.plan && (
        <div className="acc-alert warning">
          <strong>Downgrade scheduled</strong> — moving to {sub.scheduledTier} at next billing cycle
      ...
      {/* Paid: manage */}
      {!tier.isAdmin && tier.plan !== 'free' && !sub.cancelAtPeriodEnd && (
        <div className="acc-box">
  ...
  return (
    <div className="acc-section">
      <p className="acc-hint">
        Each row represents one uploaded bank statement. Deleting a batch removes all its transactions — manual entries are unaffected.
      </p>
...
function ExportSection({ user }) {
  ...
  return (
    <div className="acc-section">
      <p className="acc-hint">Export your transaction data as CSV or Excel. The XLSX file includes a summary sheet by category.</p>
...
function DataSection({ user, onClose }) {
  ...
  return (
    <div className="acc-section">
      <p className="acc-hint">
        Your financial data is encrypted in transit and stored securely. It is never sold to third parties or used for advertising.
      </p>
      ...
      <div className="acc-danger-box">
        ...
        <div className="acc-danger-desc">
          Permanently removes your profile, all uploaded statements, transaction history, and categorisation rules.
        ...
        {deleteStep === 1 && (
          <div className="acc-delete-warning">
            <div className="acc-delete-warning-text">
              <strong>This is permanent and irreversible.</strong> All your transactions, categories, rules, and profile data will be deleted immediately. Your subscription will be cancelled.
            </div>
            <div className="acc-confirm-btns">
