...
export const SA_RULES = [
  ...
  { patterns: [
    ...
    'bidorbuy', 'gumtree',
    'payfast*', 'paygate*',   // generic online payments — likely shopping
  ], category: 'Other' },
...
export function normalizeDescription(description) {
  ...
  return description
    .replace(/^(FLW\*|PAYGATE\*|PAYFAST\*|YOCO\*|SQ \*|POS\*|CARD\s*PAYMENT\s*[-–]?\s*)/i, '')
    .replace(/\s{2,}/g, ' ')
    .replace(/\d{8,}/g, '')    // strip long reference numbers
    .trim()
}
...
export function saPreCategory(description) {
  ...
  // Yoco wildcard — anything remaining from Yoco is likely a small business; leave as Other
