...
- Transaction amounts are stored as **rands** (not cents) in the `transactions` table. Profile fields (`net_income`, `gross_income`, `monthly_debit_orders`, `savings_goal`) are stored as **integer cents**. These are different units. Any code that mixes them must apply `/100` to profile fields before comparison or display. `budget-chat.js` currently divides transaction amounts by 100, which produces values 100x too small — this is a latent bug outside the current fix scope.
